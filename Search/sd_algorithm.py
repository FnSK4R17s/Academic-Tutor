...
import copy
import nltk
from Search.region import Region
from lxml import etree, html
from lxml.html import HtmlComment
from lxml.html.clean import Cleaner
from Search.terminal_colors import Tcolors
...
class SDAlgorithm():
        ...
        if article_exists:
            ...
            if max_group in grouped_comments:
                ...
                if self.big_areas_in_same_level(article, grouped_comments, max_group) and not validated:
                    ...
                    print("Class: ")
                    print(Tcolors.RES + " " +
                          grouped_comments[max_group][0].class_name)
                    print("Texts: ")
        ...
        if grouped_comments is not None:
            ...
            for group in groups_tuple:
                if group[1][0].root != article.root:
                    comment_parent_path = self.get_path(
                        group[1][0].root_node.getparent())
                    common_path = self.extract_common(
                        article_parent_path, comment_parent_path)
    ...
    def find_group_density(self, groups):
        ...
        for region in groups:
            for content in region.contents:
                content_text = region.root_node.xpath(
    ...
    def candidate_context_validated(self, article, grouped_comments, max_group):
        ...
        COMMENT_TAGS = ['comment', 'reply', 'response',
                        'ident', 'said:', 'rate', 'user', 'inner', 'wrote:']
        ...
        for des in list(comment_parent.iterdescendants()) + [comment_parent]:
            ...
            for ctag in COMMENT_TAGS:
                ...
                if contents.count(ctag) > 0:
                    ...
                    detected_on_attributes += (classname +
                                               id).lower().count(ctag)
    ...
    def print_article(self, article):
        ...
        """
        def save_article(article, destination, filename):
            if not os.path.exists(destination):
                os.makedirs(destination)
            with open("{}/{}.txt".format(destination, filename), 'w',encoding='utf-8') as f:
                # print(Tcolors.INFO + " Article detected!")
                # print("Article class: ")
                # f.write(Tcolors.RES + " " + repr(article.class_name))
                # print("Article title: ")
                # f.write(article.get_ancestor_title())
                print("Writing to file: ")
                f.write(article.full_text.replace("\n", " "))
        ...
        save_article(article, self.destination, self.filename)
        ...
        for group in tmp_nodes:
            ...
            if parent is not None:
                ...
                if parent_path in self.valid_nodes:
                    ...
                    self.valid_nodes[parent_path].extend(
                        self.valid_nodes[group])
    ...
    def close_diff_from_second_max(self):
        ...
        for region in self.regions:
            if region.root != self.max_region.root:
                d = ((float)(region.density)/(float)
                     (self.max_region.density)) * 100
                if 100 - d <= 20:
                    return True
        return False
    ...
    def find_node_text(self, node):
        ...
        if t and node.text is not None:
            ...
            try:
                ...
                itertext = []
            itertexts = [text for text in itertext if text is not None and re.sub(
                r"\n|\r|\t| |,|\.", "", text) != ""]
